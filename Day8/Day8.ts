...
function checkTrees(numMatrix: number[][]) {
  ...
  for (let row = 1; row < numMatrix.length - 1; row++) {
    for (let cell = 1; cell < numMatrix[row].length - 1; cell++) {
      ...
      const currentColMax = Math.max(
        ...numMatrix.map((val, i) => {
          return i < row ? val[cell] : 0;
        })
      );
      const remainingColMax = Math.max(
        ...numMatrix.map((val, i) => {
          return i > row ? val[cell] : 0;
        })
      );
...
function checkTrees2(numMatrix: number[][]) {
  ...
  for (let row = 1; row < numMatrix.length - 1; row++) {
    for (let cell = 1; cell < numMatrix[row].length - 1; cell++) {
      ...
      const currentCol = numMatrix.slice(0, row).map((val, i) => {
        return val[cell];
      });
      const remainingCol = numMatrix.slice(row + 1).map((val, i) => {
        return val[cell];
      });
      const rows = [
        currentRow.reverse(),
